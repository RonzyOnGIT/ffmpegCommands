...
int change_audio_to_japanese(DIR *dir, char *prefixPath) {
    ...
    // first store original name of the file, then change the original file name to outputfile.mkv, then use command, then change output filename to original, then delete the original file
    while ((entry = readdir(dir)) != NULL) {
        ...
        // plus 5 includes one for null terminating and 4 for two pairs of '""'
        ...
        // An edgecase to think about for future testing: sometimes the language will be 'und'
        int index_is_one = false;
        int japanese_audio_track_idx = -1; // which audio track index is the japanese one (zero-index). In the end, I will use this index to change this audio index to default of 1, hence reason for tracking index value
        int curr_audio_track_idx = -1;
        int current_default_idx = -1; // the track that is the default stream
        ...
        while (fgets(buffer, sizeof(buffer), fP) != NULL) {
            ...
            if (default_value == 1) {
                current_default_idx = curr_audio_track_idx;
            }

            // I'm missing one more thing
            // If I get to a track with default audio set to 1, then I need to set it to zero
        ...
        // only performs audio track change, if default audio is not japanese and japanese track found
        // an existing japanese track can exist but if marked as 'und' in ffprobe then can miss this case
        if (!is_default_jpn && japanese_audio_track_idx > -1) {
            ...
            snprintf(command, sizeof(command), "ffmpeg -i -map 0 -c copy -disposition:a:%d 0 -disposition:a:%d default output.mkv", current_default_idx, japanese_audio_track_idx);
            ...
            int commandCount = get_word_count("ffmpeg -i -map 0 -c copy -disposition:a:0 0 -disposition:a:1 default ") + 2;
            ...
            printf("file already japanese or could not find japanese track\n");
        }
